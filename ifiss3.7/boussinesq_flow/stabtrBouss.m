...
flag = 0; nrej=0; avflag = 0;  nav=nstar; tstar=1e-6;
avstep=0;
%
...
%----------------------------------- stabilized version
        if (domain==7| domain==10) & norm(xns,inf)/norm(s,inf) > 1e7; fprintf('oops .. near singularity')
...
%--- correction       u  = resetflowbc(u,xyv,boundv,t); % update boundary values
       switch(bsn)
          ...
          case{4}   %   loop
             tt = resetheatbcl(tt,bnd_dh,bnd_dc,t);
       end
       avflag=1;
